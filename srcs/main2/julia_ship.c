...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/02/12 16:27:29 by lde-batz          #+#    #+#             */
/*   Updated: 2019/02/18 13:03:37 by lde-batz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/fractol2.h"

void	*ft2_thr_ship2(void *param)
{
	t_thread	*f;
...
void	ft2_init_julia_ship_thr(t_fractol *f, t_img *img, t_thread thr[THREAD])
{
	int	i;
...
void	ft2_print_julia_ship(void *param, t_img *img, t_fractol *f)
{
	...
	win = param;
	ft2_init_julia_ship_thr(f, img, thread);
	i = -1;
	while (++i < THREAD)
		pthread_create(&f->ptr_thread[i], NULL, ft2_thr_ship2, &thread[i]);
	i = -1;
	while (++i < THREAD)
...
void	ft2_init_julia_ship(void *param)
{
	...
	win->fract.y1 = -2.0;
	win->fract.y2 = 2.0;
	ft2_print_julia_ship(win, &win->img, &win->fract);
}
