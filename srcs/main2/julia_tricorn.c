...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/02/12 16:25:42 by lde-batz          #+#    #+#             */
/*   Updated: 2019/02/18 13:04:56 by lde-batz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/fractol2.h"

void	*ft2_thr_tricorn2(void *param)
{
	t_thread	*f;
...
void	ft2_init_julia_tri_thr(t_fractol *f, t_img *img, t_thread thr[THREAD])
{
	int	i;
...
void	ft2_print_julia_tricorn(void *param, t_img *img, t_fractol *f)
{
	...
	win = param;
	ft2_init_julia_tri_thr(f, img, thread);
	i = -1;
	while (++i < THREAD)
		pthread_create(&f->ptr_thread[i], NULL, ft2_thr_tricorn2, &thread[i]);
	i = -1;
	while (++i < THREAD)
...
void	ft2_init_julia_tricorn(void *param)
{
	...
	win->fract.y1 = -2.0;
	win->fract.y2 = 2.0;
	ft2_print_julia_tricorn(win, &win->img, &win->fract);
}
