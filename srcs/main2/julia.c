...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/02/12 16:25:25 by lde-batz          #+#    #+#             */
/*   Updated: 2019/02/14 09:59:03 by lde-batz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/fractol2.h"

void	*ft2_thr_julia2(void *param)
{
	t_thread	*f;
...
void	ft2_init_julia_thread(t_fractol *f, t_img *img, t_thread thread[THREAD])
{
	int	i;
...
void	ft2_print_julia(void *param, t_img *img, t_fractol *f)
{
	...
	win = param;
	ft2_init_julia_thread(f, img, thread);
	i = -1;
	while (++i < THREAD)
	{
		if (f->head == 2.0)
			pthread_create(&f->ptr_thread[i], NULL, ft2_thr_julia, &thread[i]);
		else
			pthread_create(&f->ptr_thread[i], NULL, ft2_thr_julia2, &thread[i]);
	}
	i = -1;
	while (++i < THREAD)
...
void	ft2_init_julia(void *param)
{
	...
	win->fract.y1 = -2.0;
	win->fract.y2 = 2.0;
	ft2_print_julia(win, &win->img, &win->fract);
}
