...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/01/14 15:50:37 by lde-batz          #+#    #+#             */
/*   Updated: 2019/02/14 09:58:28 by lde-batz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/fractol1.h"

void	*ft_thr_mandel2(void *param)
{
	t_thread	*f;
...
void	ft_init_mandel_thread(t_fractol *f, t_img *img, t_thread thread[THREAD])
{
	int	i;
...
void	ft_print_mandel(void *param, t_img *img, t_fractol *f)
{
	...
	win = param;
	ft_init_mandel_thread(f, img, thread);
	i = -1;
	while (++i < THREAD)
	{
		if (f->head == 2.0)
			pthread_create(&f->ptr_thread[i], NULL, ft_thr_mandel, &thread[i]);
		else
			pthread_create(&f->ptr_thread[i], NULL, ft_thr_mandel2, &thread[i]);
	}
	i = -1;
	while (++i < THREAD)
...
void	ft_init_mandel(void *param)
{
	...
	win->fract.y1 = -2.0;
	win->fract.y2 = 2.0;
	ft_print_mandel(win, &win->img, &win->fract);
}
