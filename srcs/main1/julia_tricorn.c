...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/02/07 15:58:04 by lde-batz          #+#    #+#             */
/*   Updated: 2019/02/18 13:05:36 by lde-batz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/fractol1.h"

void	*ft_thr_tricorn2(void *param)
{
	t_thread	*f;
...
void	ft_init_julia_tri_thr(t_fractol *f, t_img *img, t_thread thr[THREAD])
{
	int	i;
...
void	ft_print_julia_tricorn(void *param, t_img *img, t_fractol *f)
{
	...
	win = param;
	ft_init_julia_tri_thr(f, img, thread);
	i = -1;
	while (++i < THREAD)
		pthread_create(&f->ptr_thread[i], NULL, ft_thr_tricorn2, &thread[i]);
	i = -1;
	while (++i < THREAD)
...
void	ft_init_julia_tricorn(void *param)
{
	...
	win->fract.y1 = -2.0;
	win->fract.y2 = 2.0;
	ft_print_julia_tricorn(win, &win->img, &win->fract);
}
