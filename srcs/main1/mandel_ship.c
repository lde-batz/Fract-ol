...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/02/07 16:04:48 by lde-batz          #+#    #+#             */
/*   Updated: 2019/02/18 13:02:05 by lde-batz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/fractol1.h"

void	*ft_thr_ship1(void *param)
{
	t_thread	*f;
...
void	ft_init_mandel_ship_thr(t_fractol *f, t_img *img, t_thread thr[THREAD])
{
	int	i;
...
void	ft_print_mandel_ship(void *param, t_img *img, t_fractol *f)
{
	...
	win = param;
	ft_init_mandel_ship_thr(f, img, thread);
	i = -1;
	while (++i < THREAD)
		pthread_create(&f->ptr_thread[i], NULL, ft_thr_ship1, &thread[i]);
	i = -1;
	while (++i < THREAD)
...
void	ft_init_mandel_ship(void *param)
{
	...
	win->fract.y1 = -2.0;
	win->fract.y2 = 2.0;
	ft_print_mandel_ship(win, &win->img, &win->fract);
}
